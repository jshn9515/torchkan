...
class KANConvNDLayer(nn.Module):
    def __init__(
        ...
        layer_norm = norm_class(num_features=out_channels // groups, **norm_kwargs)
    ...
    def forward_kan(self, x: Tensor, group_index: int) -> Tensor:
        ...
        bases = torch.logical_and(x_uns >= grid[..., :-1], x_uns < grid[..., 1:]).to(x.dtype)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        for group_index, x in enumerate(split_x):
            x = self.forward_kan(x, group_index)
            output.append(x)
