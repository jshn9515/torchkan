...
class KANLayer(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        grid = self.grid.to(x.device)  # Move the input tensor to the device where the weights are located.

        # Perform the base linear transformation followed by the base_activation function.
        base_output = F.linear(self.base_activation(x), self.base_weight)
        x_uns = torch.unsqueeze(x, -1)  # Expand dimensions for spline operations.
...
class KALNLayer(nn.Module):  # Kolmogorov Arnold Legendre Network (KAL-Net)
    def __init__(
        ...
        self.poly_weight = nn.Parameter(torch.randn(out_features, in_features * (degree + 1)))
...
class FastKANLayer(nn.Module):
    def __init__(
        ...
        self.spline_linear = SplineLinear(in_features * num_grids, out_features,
...
class ChebyKANLayer(nn.Module):
    def __init__(self, in_features: int, out_features: int, degree: int):
        ...
        self.cheby_coeff = nn.Parameter(torch.empty(in_features, out_features, degree + 1))
        nn.init.normal_(self.cheby_coeff, mean=0.0, std=1 / (in_features * (degree + 1)))
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        y = torch.einsum('bid,iod->bo', x, self.cheby_coeffs)  # shape = (batch_size, out_features)
...
class WavKANLayer(nn.Module):
    def __init__(self, in_features: int, out_features: int, wavelet_type: str = 'mexican_hat'):
...
class JacobiKANLayer(nn.Module):
    def __init__(
        ...
        self.jacobi_coeff = nn.Parameter(torch.empty(in_features, out_features, degree + 1))

        nn.init.normal_(self.jacobi_coeff, mean=0.0, std=1 / (in_features * (degree + 1)))
        nn.init.xavier_uniform_(self.base_weights)

    def forward(self, x: Tensor) -> Tensor:
        x = torch.reshape(x, (-1, self.in_features))  # shape = (batch_size, in_features)
        ...
        jacobi = torch.ones(x.shape[0], self.in_features, self.degree + 1, device=x.device)
        if self.degree > 0:  # degree = 0: jacobi[:, :, 0] = 1 (already initialized); degree = 1: jacobi[:, :, 1] = x;
        ...
        y = torch.einsum('bid,iod->bo', jacobi, self.jacobi_coeffs)  # shape = (batch_size, out_features)
        y = y.view(-1, self.out_features)
...
class BernsteinKANLayer(nn.Module):
    def __init__(
        ...
        self.bernstein_coeff = nn.Parameter(torch.empty(in_features, out_features, degree + 1))
        self.base_activation = base_activation

        nn.init.normal_(self.bernstein_coeffs, mean=0.0, std=1 / (in_features * (degree + 1)))
        nn.init.xavier_uniform_(self.base_weights)
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        y = torch.einsum('bid,iod->bo', bernsteins, self.bernstein_coeffs)  # shape = (batch_size, out_features)
        y = y.view(-1, self.out_features)
...
class ReLUKANLayer(nn.Module):
    def __init__(
        ...
        # modification here
        self.phase_low = nn.Parameter(phase_low[None, :].expand(in_channels, -1), requires_grad=train_ab)
        # modification here, and: `phase_height` to `phase_high`
        self.phase_high = nn.Parameter(phase_high[None, :].expand(in_channels, -1), requires_grad=train_ab)
        self.equal_size_conv = nn.Conv2d(1, out_channels, kernel_size=(g + k, in_channels))
...
class BottleNeckGRAMLayer(nn.Module):
    def __init__(
        ...
        inner_dim = int(max(in_features / dim_reduction,
                            out_features / dim_reduction))
